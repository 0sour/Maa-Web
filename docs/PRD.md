...
| T-01~T-03 工具箱识别 | ✅ 第一批已实现 | **公招/仓库/干员识别**（Recruit 纯识别模式 confirm=[-1] / Depot / OperBox）+ 识别结果历史记录 + 按结果执行招募联动（2026-08-16）；**2026-08-17 真机首测通过**（公招组合/75 种材料/286 名干员） |
| T-05 抽卡 / T-06 窥屏 | 🚧 规划中 | 工具箱页「执行工具」占位；抽卡安全流程（设置开关 + 双弹窗 5s 确认）已定稿待实现 |
| C-04 / S-08 外服（部分）/ D-12~D-15 | ⏳ 未排期 | 见 roadmap M4/M5/M6 |
