...
| S-01 任务编排 | ✅ v1 已实现 | 拖拽排序/勾选启用/「仅一次」/LINK START/STOP/方案「任务文件」保存调出/草稿自动留底；两页复用同一套组件 |
| S-02 定时执行 | ✅ 已实现 | `schedule_jobs` 表 + 分钟级调度器（整点 tick/90s 防重）+ /schedule 页（方案快照/星期/时间/试跑/草稿留底）；**实机整点触发验证通过** |
