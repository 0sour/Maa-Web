...
| 工具箱 | `ToolboxView` | 工具导航：公招识别/干员识别/仓库识别（已上线，识别结果 + 历史记录 + 招募联动；历史记录**不分设备展示**、行内标注设备、面板设备筛选下拉；干员结果**高星→低星 + 同星按名称排序** + 星级筛选下拉）；抽卡/窥屏为「规划中」占位（高危功能设置开关 + 双弹窗 5s 确认已定稿）；小游戏未排期 |
