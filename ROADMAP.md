...
- **定时任务**：周几+时间点调度、错过补偿（3h）、下次执行预览、**执行内容可选「任务队列」或「单任务」**（2026-08-06：单任务=快速任务同款类型选择+参数表单，触发时 buildArgs 直跑 maa-cli，支持后置动作；编辑回填、卡片显示「单任务：类型」；保存时校验任务类型合法性；配置界面与快速任务一致——肉鸽模式/分队/难度选项按主题动态加载（theme 切换联动清空）、抄作业支持作业集自动展开列表+勾选执行）
