...
- [ ] **T 今日关卡按凌晨 4 点切换（记录于 2026-08-06）**：
  - 需求：任务队列上方的「每日开放副本」按游戏日边界更新——**每天凌晨 4:00 更新为下一天**的开放关卡（明日方舟每日刷新时间）
  - 现状：`server/stages.js` 的 `today()` 用 `new Date().getDay()`（自然日 0 点切换），凌晨 0-4 点之间显示的是旧一天还是新一天与游戏不一致（应显示游戏日：4:00 前仍算前一天）
  - 实施：`today()` 计算 weekDay 前先把时间减去 4 小时（或 `if (now.getHours() < 4) day = previousDay`），活动关卡同理（活动数据如果也是按天区分需一致处理）；注意容器 TZ=Asia/Shanghai（R 已修复）
  - 成本：低
