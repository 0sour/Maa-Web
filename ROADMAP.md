...
- **关卡信息自动刷新**（2026-08-05 完成）：队列页停留时每 10 分钟轮询 `/api/stages/today`，跨天自动切换
...
- **架构**：纯轮询（5s）无 SSE、静态资源版本号缓存策略、HTML 标签完整性校验（严格解析器）、jsdom + 服务端集成测试（16 个文件全绿）
...
- [ ] **D 多配置/后置动作**：多套配置切换 + 定时器关联 profile；任务完成后退出模拟器/休眠/关机
